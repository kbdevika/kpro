...
testRouter.post('/', async (req: any, res: any) => {
    try {
        ...
        const response = await fetch('https://ondc-api.kiko.live/ondc-seller-v2/kiranaProSearch', {
          ...
          body: JSON.stringify({ pincode: req.body.pincode }),
        });
        ...
        const newCatalogue = await prisma.catalogue.upsert({
          where: {
            pincode: req.body.pincode,  // Assuming 'pincode' is unique for each catalogue
          },
          update: {
            jsonData: storeData,  // If the catalogue exists, update its jsonData
          },
          create: {
            pincode: req.body.pincode,  // If the catalogue doesn't exist, create a new one
            jsonData: storeData,
          },
        });
        return data;
